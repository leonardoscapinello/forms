  ...
  return (
    <div>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ top: 58, right: 16, left: 0, bottom: 8 }} style={{ overflow: 'visible' }}>
  ...
  return (
    <div>
      <ResponsiveContainer width="100%" height={Math.max(300, items.length * 50)}>
        <BarChart data={data} layout="vertical" margin={{ top: 14, right: 80, left: 10, bottom: 10 }} style={{ overflow: 'visible' }}>
          <GradientDefs items={items} />
...
function LineChart({ items, style }: { items: GraphicDataItem[]; style: ChartStyle }) {
  ...
  return (
    <div>
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data} margin={{ top: 58, right: 16, left: 0, bottom: 8 }} style={{ overflow: 'visible' }}>
          <defs>
            <filter id="tooltipCardShadow" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="1" stdDeviation="2" floodColor="#000" floodOpacity="0.08" />
            </filter>
