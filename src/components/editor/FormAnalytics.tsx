      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={sessionsByDay} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="sessions" stroke="hsl(var(--primary))" fill="url(#gradSessions)" strokeWidth={2} name="Sessões" dot={false} />
            <Area type="monotone" dataKey="completed" stroke="hsl(142 71% 45%)" fill="url(#gradCompleted)" strokeWidth={2} name="Concluídos" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className={`rounded-lg p-3 ${isLast ? 'bg-primary/5 border border-primary/20' : 'bg-muted/30'}`}>
      ...
      <div className="h-2 rounded-full bg-background overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${isLast ? 'bg-primary' : 'bg-primary/60'}`}
