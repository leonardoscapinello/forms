...
export default function ABTestNode({ data }: Props & { data: Props['data'] & { isNodeDisabled?: boolean } }) {
  ...
  const addVariant = () => {
    ...
    const updated = [...variants.map(v => ({ ...v, weight })), { ...newVariant, weight }];
    // Fix rounding
    const diff = 100 - updated.reduce((s, v) => s + v.weight, 0);
  ...
  const removeVariant = (id: string) => {
    ...
    const updated = remaining.map(v => ({ ...v, weight }));
    const diff = 100 - updated.reduce((s, v) => s + v.weight, 0);
    if (diff !== 0) updated[0].weight += diff;
    onChange({ variants: updated });
  };
  ...
  return (
    <div className={`bg-card rounded-xl border border-node-abtest-accent/30 shadow-sm w-72 overflow-hidden ${isNodeDisabled ? 'opacity-50 grayscale' : ''}`}>
      ...
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2.5 bg-node-abtest border-b border-node-abtest-accent/20">
        <div className="h-6 w-6 rounded-md bg-node-abtest-accent/20 flex items-center justify-center">
